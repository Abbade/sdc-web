...
const createObjFormSchema = yup.object().shape({
  ...
  variableTypeId: yup.number().required("Tipo Variação é obrigatório")
});

interface VariationValueProps extends EditInterface{
  variationTypeId?: number;
}

export default function VariationValueForm({ id, onClose, variationTypeId }: VariationValueProps) {
  ...
  useEffect(() => {
    console.log(variationTypeId);
    if(variationTypeId){
      setValue("variableTypeId", variationTypeId);
    }
  }, [variationTypeId, setValue])
    ...
    const get = async (id) => {
      ...
      try {
        if (id > 0) {
          ...
          const item = await api.get(`variableTypeValues/${id}`);
  ...
  const handleLoteSubmit: SubmitHandler<CreateFormData> = async (formData) => {
    try {
      ...
      if (formData.id > 0) {
        const item = await api.put("variableTypeValues", formData);
        setOpenLoading(false);
        showAlert("Variação editada com sucesso.", "success");
      } else {
        const item = await api.post("variableTypeValues", formData);
        setOpenLoading(false);
    ...
    <Container component="main" maxWidth="xs">
      ...
      >
        ...
        <Button
          ...
          sx={{ mt: 3, mb: 2 }}
        >
          {id != undefined ? "Editar" : "Cadastrar"} Valor da Variação
        </Button>
      </Box>
    </Container>
  );
