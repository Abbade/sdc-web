...
export default function CreateTrashReasonForm({ id, onClose }: EditInterface) {
  const {
    handleSubmit,
    control,
    setValue,
  ...
  useEffect(() => {
    const get = async (id) => {
      if (id > 0) {
        setOpenLoading(true);
        const item = await api.get(`trash-reason/${id}`);
  ...
  const handleLoteSubmit: SubmitHandler<CreateGeneticProfileFormData> = async (
    formData
      ...
      try {
        setOpenLoading(true);
        if (formData.id > 0) {
          const item = await api.put("trash-reason", formData);
          showAlert("Motivo Descarte editado com sucesso.", "success");
        } else {
          const item = await api.post("trash-reason", formData);
          showAlert("Motivo Descarte cadastrado com sucesso.", "success");
        }
        setOpenLoading(false);
        onClose(true);
      } catch (error) {
