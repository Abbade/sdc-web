...
export default function CreateSectionForm({ id, onClose }: EditInterface) {
  const {
    handleSubmit,
    control,
    setValue,
  ...
  useEffect(() => {
    const get = async (id) => {
      if (id > 0) {
        setOpenLoading(true);
        const item = await api.get(`section/${id}`);
  ...
  const handleLoteSubmit: SubmitHandler<CreateGeneticProfileFormData> = async (
    ...
    try {
      try {
        setOpenLoading(true);
        if (formData.id > 0) {
          const item = await api.put("section", formData);
          showAlert("Seção editada com sucesso.", "success");
        } else {
          const item = await api.post("section", formData);
          showAlert("Seção cadastrada com sucesso.", "success");
        }
        setOpenLoading(false);
        onClose(true);
      } catch (error) {
