...
export default function ActionLotesTable({ id }) {
  ...
  useEffect(() => {
    ...
    const get = async (name: string, page: number, pageSize: number) => {
      var response = await api.get("/action-lotes", {
        params: {
          id: id,
        },
      });
...
const columns: GridColDef[] = [
  ...
  { field: "qt", headerName: "Quantidade", width: 160 },
  {
    ...
    renderCell: (params) => {
      return (
        <div className="MuiDataGrid-cellContent">
          {params.row.action.name === "Descartar mudas"? params.row.trashReason?.name : params.row.location?.name}
        </div>
      );
    },
  },
